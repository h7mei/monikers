...
import { z } from 'zod';
import { roomManager, GameRoom, Player } from './roomManager';
...
export const roomRouter = router({
  ...
  joinRoom: publicProcedure
    ...
    .mutation(({ input }) => {
      const player = roomManager.joinRoom(input.roomId, input.playerName, input.deviceType);
  ...
  updateGameState: publicProcedure
    ...
    .mutation(({ input }) => {
      const success = roomManager.updateGameState(input.roomId, input.gameState);
