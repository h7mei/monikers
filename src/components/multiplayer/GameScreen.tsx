...
export default function MultiplayerGameScreen({ roomId, player, onGameEnd }: Props) {
  ...
  const [scores, setScores] = useState<Record<string, Record<number, Card[]>>>({ team1: {}, team2: {} });
  ...
  const [playerSkipCounts, setPlayerSkipCounts] = useState<Record<string, number>>({});
  ...
  const getNextCard = (currentPlayer: Player): Card | null => {
    ...
    const teamCards = allCards.filter(card => {
      ...
      return playerWithCard?.team === currentTeam && !usedCards.has(card.text) && !usedCards.has(card.id);
    });
  ...
  const skipCurrentCard = () => {
    ...
    const currentCardIndex = teamCards.findIndex(card =>
      card.id === currentCard.id || card.text === currentCard.text
    );
  ...
  const handleCorrectGuess = () => {
    ...
    setScores(newScores);
    const newUsedCards = new Set([...usedCards, currentCard.text || currentCard.id]);
    ...
    const currentPlayer = room.players[currentPlayerIndex];
    if (currentPlayer) {
      const newPlayerSkipCounts = { ...playerSkipCounts, [currentPlayer.id]: 0 };
  ...
  const handleSkipCard = () => {
    ...
    if (currentPlayer) {
      ...
      const newPlayerSkipCounts = { ...playerSkipCounts, [currentPlayer.id]: newSkipCount };
      setPlayerSkipCounts(newPlayerSkipCounts);
  ...
  const moveToNextPlayer = () => {
    ...
    const currentTeamPlayers = room.players.filter(p => p.team === currentTeam);
    ...
    const currentPlayerInTeamIndex = currentTeamPlayerIndices.indexOf(currentPlayerIndex);
    const nextPlayerInTeamIndex = (currentPlayerInTeamIndex + 1) % currentTeamPlayerIndices.length;
    ...
    if (nextPlayerInTeamIndex === 0) {
      ...
      if (newTeamPlayers.length > 0) {
        const firstNewTeamPlayerIndex = room.players.findIndex(p => p.id === newTeamPlayers[0].id);
  ...
  const getTeamScore = (team: string, round: number) => {
    return scores[team]?.[round]?.reduce((sum, card) => {
      // Handle both Card interface (from roomManager) and GameCard interface (from GameScreen)
      const cardLevel = (card as any).level || 1; // Default to level 1 if level is not available
  ...
  const getTotalTeamScore = (team: string) => {
    return [1, 2, 3].reduce((total, round) => total + getTeamScore(team, round), 0);
  };
  ...
  const areAllQuestionsAnswered = () => {
    ...
    const answeredCards = getTeamCardCount('team1', currentRound) + getTeamCardCount('team2', currentRound);
  ...
  }, [currentPlayerIndex, currentTeam, roundStarted, isRoundActive, currentCard, room]);
  ...
  const roundDescription = `Round ${currentRound}: ${currentRound === 1 ? 'Free Talking' :
    currentRound === 2 ? 'One Word' :
      'Expressions'
    }`;
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-6">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl font-bold mb-8">Round {currentRound} Complete!</h1>

          <div className="bg-yellow-500/20 p-6 rounded-lg mb-8">
            <h2 className="text-2xl font-bold text-yellow-400 mb-4">All Questions Answered!</h2>
            <p className="text-lg text-gray-300">Both teams have answered all their questions for this round.</p>
          </div>

          <div className="grid grid-cols-2 gap-8 mb-8">
            <div className="bg-blue-500/20 p-6 rounded-lg">
              <h2 className="text-2xl font-bold text-blue-400 mb-4">Team 1</h2>
              <div className="text-4xl font-bold">{getTeamCardCount('team1', currentRound)}</div>
            ...
            <div className="bg-green-500/20 p-6 rounded-lg">
              ...
              <div className="text-4xl font-bold">{getTeamCardCount('team2', currentRound)}</div>
              <p className="text-gray-400">cards answered</p>
            </div>
          </div>

          <div className="bg-gray-800 p-6 rounded-lg mb-8">
            <h3 className="text-xl font-bold mb-4">Round {currentRound} Summary</h3>
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <p className="text-gray-400">Team 1 Score</p>
                <p className="text-2xl font-bold text-blue-400">{getTeamScore('team1', currentRound)}</p>
              </div>
              <div>
                <p className="text-gray-400">Team 2 Score</p>
                <p className="text-2xl font-bold text-green-400">{getTeamScore('team2', currentRound)}</p>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-gray-700">
              <p className="text-gray-400">Round Winner</p>
              <p className="text-xl font-bold text-yellow-400">
                {getTeamScore('team1', currentRound) > getTeamScore('team2', currentRound) ? 'Team 1' :
                  getTeamScore('team2', currentRound) > getTeamScore('team1', currentRound) ? 'Team 2' : 'Tie'}
              </p>
            </div>
          </div>
            ...
            <div className="mb-8">
              <button
                ...
                onClick={() => {
                  ...
                  // Set initial team
                  const firstTeam1Player = room.players.find(p => p.team === 'team1');
                  if (firstTeam1Player) {
                    const playerIndex = room.players.findIndex(p => p.id === firstTeam1Player.id);
              ...
              <div className="bg-gray-800 p-6 rounded-lg mb-8">
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div className="bg-blue-500/20 p-4 rounded-lg">
                    <h4 className="text-lg font-bold text-blue-400 mb-3">Team 1</h4>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>Round 1:</span>
                        <span className="font-bold">{getTeamScore('team1', 1)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Round 2:</span>
                        <span className="font-bold">{getTeamScore('team1', 2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Round 3:</span>
                        <span className="font-bold">{getTeamScore('team1', 3)}</span>
                      </div>
                      <div className="border-t pt-2 flex justify-between">
                        <span className="font-bold">Total:</span>
                        <span className="font-bold text-blue-400">{getTotalTeamScore('team1')}</span>
                      </div>
                    </div>
                  </div>

                  <div className="bg-green-500/20 p-4 rounded-lg">
                    <h4 className="text-lg font-bold text-green-400 mb-3">Team 2</h4>
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <span>Round 1:</span>
                        <span className="font-bold">{getTeamScore('team2', 1)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Round 2:</span>
                        <span className="font-bold">{getTeamScore('team2', 2)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Round 3:</span>
                        <span className="font-bold">{getTeamScore('team2', 3)}</span>
                      </div>
                      <div className="border-t pt-2 flex justify-between">
                        <span className="font-bold">Total:</span>
                        <span className="font-bold text-green-400">{getTotalTeamScore('team2')}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-yellow-500/20 p-4 rounded-lg mb-6">
                  <h4 className="text-xl font-bold text-yellow-400 mb-2">🏆 Game Winner</h4>
                  <p className="text-2xl font-bold text-yellow-400">{getWinner()}</p>
                </div>

                {/* Detailed Card Report */}
                <div className="bg-gray-700 p-4 rounded-lg">
                  <h4 className="text-lg font-bold mb-4">📋 Detailed Card Report</h4>
                  <div className="space-y-4">
                    {[1, 2, 3].map(round => (
                      <div key={round} className="border border-gray-600 rounded-lg p-3">
                        <h5 className="font-bold text-lg mb-2">Round {round}: {round === 1 ? 'Free Talking' : round === 2 ? 'One Word' : 'Expressions'}</h5>

                        {/* Team 1 Cards */}
                        <div className="mb-3">
                          <h6 className="font-semibold text-blue-400 mb-1">Team 1 Cards:</h6>
                          {scores['team1']?.[round]?.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                              {scores['team1'][round].map((card, index) => {
                                ...
                                return (
                                  <div key={index} className="bg-blue-500/10 p-2 rounded text-sm">
                                    <span className="font-semibold">"{card.text}"</span>
                                    <br />
                                    <span className="text-gray-400 text-xs">
                                      Selected by: {playerWithCard?.name || 'Unknown'}
                                    </span>
                                    <br />
                                    <span className="text-yellow-400 text-xs">
                                      Level {(card as any).level || 1} ({((card as any).level || 1)} points)
                                    </span>
                                  </div>
                                );
                              })}
                            ...
                            <p className="text-gray-400 text-sm">No cards answered</p>
                          )}
                        </div>

                        {/* Team 2 Cards */}
                        <div>
                          <h6 className="font-semibold text-green-400 mb-1">Team 2 Cards:</h6>
      ...
      <div className="min-h-screen bg-gray-900 text-white p-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 gap-8 mb-8">
            <div className="bg-blue-500/20 p-6 rounded-lg">
              <h2 className="text-2xl font-bold text-blue-400 mb-4">Team 1</h2>
              <div className="text-4xl font-bold">{getTeamScore('team1', currentRound)}</div>
              <p className="text-gray-400">cards this round</p>
            </div>
            <div className="bg-green-500/20 p-6 rounded-lg">
              <h2 className="text-2xl font-bold text-green-400 mb-4">Team 2</h2>
              <div className="text-4xl font-bold">{getTeamScore('team2', currentRound)}</div>
              <p className="text-gray-400">cards this round</p>
            </div>
          </div>

          <div className="text-xl text-gray-400">
            {currentRound < 3 ? 'Preparing for next round...' : 'Game complete!'}
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 gap-8 mb-8">
            <div className="bg-blue-500/20 p-6 rounded-lg">
              ...
              <div className="text-3xl font-bold mb-2">{getTotalTeamScore('team1')}</div>
            ...
            <div className="bg-green-500/20 p-6 rounded-lg">
              ...
              <div className="text-3xl font-bold mb-2">{getTotalTeamScore('team2')}</div>
              <p className="text-gray-400">total points</p>
              <div className="mt-4 space-y-1">
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <div className={`inline-block px-6 py-3 rounded-full text-lg font-semibold ${currentTeam === 'team1'
        ...
        {!roundStarted && !isHost && (
          <div className="text-center mb-8">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4"></div>
            <p className="text-gray-400">Waiting for host to start the round...</p>
          </div>
        )}
        ...
        {roundStarted && (
          <>
            ...
            {isMyTurn && turnStarted && (
              <div className="text-center mb-8">
                <div className={`text-6xl font-mono font-bold ${playerTimers[player.id] <= 10 ? 'text-red-500 animate-pulse' : 'text-blue-500'
                  }`}>
            ...
            <div className="bg-gray-800 p-6 rounded-lg mb-8">
              <div className="text-center">
                ...
                <p className="text-gray-400">
                  {currentPlayer?.team === 'team1' ? 'Team 1' : 'Team 2'} • {currentPlayer?.name}
                </p>
              </div>
            </div>
              ...
              <>
                {currentCard ? (
                  <div className="bg-blue-500/20 border border-blue-500 p-8 rounded-lg mb-8">
                    <div className="text-center">
                      <h3 className="text-3xl font-bold mb-4">{currentCard.text}</h3>
                      <p className="text-xl mb-4">{(currentCard as any).description}</p>
                      <p className="text-yellow-400 text-lg mb-2">
                        Level {(currentCard as any).level || 1} ({(currentCard as any).level || 1} points)
                      </p>
                      <p className="text-gray-400">
                  ...
                  <div className="bg-yellow-500/20 border border-yellow-500 p-8 rounded-lg mb-8">
                    <div className="text-center">
                      <h3 className="text-2xl font-bold mb-4">Loading Card...</h3>
                      <p className="text-gray-400">
        ...
        <div className="bg-gray-800 p-6 rounded-lg mb-8">
          <h3 className="text-center text-white font-semibold text-xl mb-4">Round {currentRound} Progress</h3>
          <div className="text-center">
            {(() => {
              const totalCards = (room?.settings.cardsPerPlayer || 0) * 2; // Total cards for both teams
              const answeredCards = getTeamCardCount('team1', currentRound) + getTeamCardCount('team2', currentRound);
            ...
            <p className="text-sm text-gray-400 mt-2">
              {getTeamCardCount('team1', currentRound) + getTeamCardCount('team2', currentRound)} / {(room?.settings.cardsPerPlayer || 0) * 2} cards answered
            </p>
          </div>
        </div>
        ...
        {showRoundSummary && (
          <div className="grid grid-cols-2 gap-6 mb-8">
            <div className="bg-blue-500/20 p-6 rounded-lg">
              <h3 className="text-center text-blue-400 font-semibold text-xl mb-4">Team 1</h3>
              <div className="text-center">
            ...
            <div className="bg-green-500/20 p-6 rounded-lg">
              <h3 className="text-center text-green-400 font-semibold text-xl mb-4">Team 2</h3>
              <div className="text-center">
          ...
          {room.players.map((p, index) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm ${p.team === 'team1' ? 'text-blue-400' : 'text-green-400'
                  }`}>
